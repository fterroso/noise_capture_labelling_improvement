...
public class MeasurementExport {
    ...
    public void exportRecord(int recordId, OutputStream outputStream, boolean exportReadme) throws IOException {
        ...
        ZipOutputStream zipOutputStream = new ZipOutputStream(outputStream); // Crea el zip
        ...
        try {
            ...
            JsonWriter mainGeo = new JsonWriter(writer);
            // Add measures
            mainGeo.beginObject(); // {
            mainGeo.name("type"); // "type": "FeatureCollection",
            mainGeo.value("FeatureCollection");
            mainGeo.name("features"); // "features": [
            mainGeo.beginArray();
            LeqJSONWriter leqJSONWriterGeo = new LeqJSONWriter(mainGeo);
            measurementManager.getRecordLocations(recordId, leqJSONWriterGeo);
            mainGeo.endArray();
            mainGeo.endObject(); // }
            mainGeo.flush();
            zipOutputStream.closeEntry();
             ...
             * TAGS
             */
            zipOutputStream.putNextEntry(new ZipEntry(sharedPref.getString("file_name_tags_" + record.getUtc(), "NONE")));
            writer = new OutputStreamWriter(zipOutputStream);
            writer.append(sharedPref.getString("tags_" + record.getUtc(), "NONE")); // Buscamos en las shared preferences
            writer.flush();
            zipOutputStream.closeEntry();
            /**
             * Para el README
